  ...
  data.frame(x = x,density = approx(den$x, den$y, xout = x)$y) |> 
    ...
    ggplot2::ggplot(ggplot2::aes(x = x, y = 0)) +  
      ggdist::stat_halfeye(
        ...
        justification = -.5, 
        point_colour = NA) + 
      ggplot2::geom_boxplot(outlier.shape = NA,box.colour = get_csdta_colours("bright_blue")) +
      ggplot2::geom_point(ggplot2::aes(size = density_scaled,alpha = density**2),color = get_csdta_colours("bright_blue")) +
